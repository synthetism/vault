...
export class File<T> extends Unit<FileProps<T>> implements IFile<T> {
  ...
  async write(): Promise<Result<void>> {
    try {
      ...
      // File saves itself!
      await this.execute('fs-async.writeFile', this.props.filename, fileContent);
  ...
  async read(): Promise<Result<T>> {
    try {
      ...
      // File loads itself!
      const content = await this.execute('fs-async.readFile', this.props.filename) as string;
  ...
  async unlink(): Promise<Result<void>> {
    try {
      if (!this.can('fs-async.unlink')) {
        return Result.fail(`[${this.props.filename}] Missing filesystem capability for deletion.`);
      }

      await this.execute('fs-async.unlink', this.props.filename);
      return Result.success(undefined);
    } catch (error) {
